...
# Definir a pasta de origem dos dados

#setwd("I:/Drives compartilhados/SUBTT - Monitoramento/CMO - Arquivos Gerais/6 - Github/R_Scripts/Temp")

setwd("G:/Drives compartilhados/SUBTT - Monitoramento/CMO - Arquivos Gerais/6 - Github/R_Scripts/Temp")


# Puxar trips para ler as circulares

gtfs <- read_gtfs("./Base de Dados/062_gtfs_janeiro_3q.zip")
...
os_parc <- read.csv(paste0("./Output/", "OS Parcial ", ano_gtfs, "-", mes_gtfs, "-", quinzena_gtfs, ".csv"),sep=';',dec=',',check.names = FALSE)
...
# Usar output do 5.2 os_por_hora_adj_3h_pf_sentido.R

partida_hora <- read.csv(paste0("./Output/", "Partidas por faixa ", ano_gtfs, "-", mes_gtfs, "-", quinzena_gtfs, ".csv"),sep=';',dec=',',check.names = FALSE)
...
#Criar coluna resolver o número de viagens no Ponto Facultativo

#os_fim$aux <- ((os_fim$`Partidas Ida - Dia Útil`+ os_fim$`Partidas Volta - Dia Útil`) / os_fim$`Viagens - Dia Útil`)
...
fwrite(os_fim, paste0("./Output/", "OS Apuração ", ano_gtfs, "-", mes_gtfs, "-", quinzena_gtfs, ".csv"),sep=';',dec=',')

# Gerando o arquivo excel
write.xlsx(os_fim,  file = paste0("./Output/", "OS Apuração ", ano_gtfs, "-", mes_gtfs, "-", quinzena_gtfs, ".xlsx"))
