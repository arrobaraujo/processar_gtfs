...
viagens_freq <- gtfs$frequencies %>%
  # Passo 1: Converter horários GTFS para segundos desde meia-noite
  mutate(
    # Extrair horas, minutos e segundos
    start_hour = as.numeric(substr(start_time, 1, 2)),
    start_min = as.numeric(substr(start_time, 4, 5)),
    start_sec = as.numeric(substr(start_time, 7, 8)),
    
    end_hour = as.numeric(substr(end_time, 1, 2)),
    end_min = as.numeric(substr(end_time, 4, 5)),
    end_sec = as.numeric(substr(end_time, 7, 8))
  ) %>%
  
  # Passo 2: Converter para segundos totais
  mutate(
    start_seconds = start_hour * 3600 + start_min * 60 + start_sec,
    end_seconds = end_hour * 3600 + end_min * 60 + end_sec
  ) %>%
  
  # Passo 3: Ajustar horários > 24 horas
  mutate(
    start_seconds_adj = if_else(start_seconds >= 86400, 
                                start_seconds - 86400, 
                                start_seconds),
    end_seconds_adj = if_else(end_seconds >= 86400, 
                              end_seconds - 86400, 
                              end_seconds)
  ) %>%
  
  # Passo 4: Converter de volta para formato HH:MM:SS
  mutate(
    start_time_clean = paste(
      sprintf("%02d", floor(start_seconds_adj / 3600)),
      sprintf("%02d", floor((start_seconds_adj %% 3600) / 60)),
      sprintf("%02d", start_seconds_adj %% 60),
      sep = ":"
    ),
    end_time_clean = paste(
      sprintf("%02d", floor(end_seconds_adj / 3600)),
      sprintf("%02d", floor((end_seconds_adj %% 3600) / 60)),
      sprintf("%02d", end_seconds_adj %% 60),
      sep = ":"
    )
  ) %>%
  
  # Passo 5: Converter para POSIXct para cálculos de duração
  mutate(
    start_posix = as.POSIXct(start_time_clean, 
                             format = "%H:%M:%S", 
                             tz = "UTC"),
    end_posix = as.POSIXct(end_time_clean, 
                           format = "%H:%M:%S", 
                           tz = "UTC")
  ) %>%
  
  # Passo 6: Ajustar para serviços que cruzam meia-noite
  mutate(
    # Se o início original era após 2h da manhã no dia seguinte
    start_posix = if_else(start_seconds >= 86400 & start_seconds < 86400 + 7200,
                          start_posix + 86400,
                          start_posix),
    # Se o fim é antes do início, adicionar um dia
    end_posix = if_else(end_posix < start_posix,
                        end_posix + 86400,
                        end_posix)
  ) %>%
  
  # Passo 7: Calcular duração e número de partidas
  mutate(
    duracao = as.numeric(difftime(end_posix, start_posix, units = "secs")),
    partidas = floor(duracao / headway_secs)
  ) %>%
  
  # Passo 8: Fazer os joins e filtros
  left_join(select(gtfs$trips, trip_id, trip_short_name, trip_headsign, 
                   direction_id, service_id)) %>%
  filter(!grepl("DESAT", service_id)) %>%
  
  # Passo 9: Adicionar metadados
  mutate(
    circular = nchar(trip_headsign) == 0,
    tipo_dia = substr(service_id, 1, 1)
  ) %>%
  
  # Passo 10: Limpar colunas temporárias
  select(-starts_with("start_hour"), -starts_with("start_min"), 
         -starts_with("start_sec"), -starts_with("end_hour"),
         -starts_with("end_min"), -starts_with("end_sec"),
         -ends_with("_seconds"), -ends_with("_seconds_adj"))
...
# Se seus horários estão no formato HH:MM:SS (padrão GTFS)
os_freq <- viagens_freq %>%
  group_by(trip_short_name, direction_id, tipo_dia) %>%
  summarise(
    partidas = sum(partidas, na.rm = TRUE),
    # Pegar os valores como texto primeiro
    inicio_texto = min(start_time, na.rm = TRUE),
    fim_texto = max(end_time, na.rm = TRUE),
    pico = min(headway_secs, na.rm = TRUE),
    circular = unique(circular)[1],
    .groups = "drop"
  ) %>%
  rename(servico = trip_short_name) %>%
  mutate(
    # Converter explicitamente com o formato conhecido
    # Adicionando uma data base para criar um POSIXct completo
    inicio = as.POSIXct(paste("1970-01-01", inicio_texto), 
                        format = "%Y-%m-%d %H:%M:%S", 
                        tz = "UTC"),
    fim = as.POSIXct(paste("1970-01-01", fim_texto), 
                     format = "%Y-%m-%d %H:%M:%S", 
                     tz = "UTC")
  ) %>%
  mutate(
    # Aplicar o timezone desejado
    inicio = lubridate::force_tz(inicio, "America/Sao_Paulo"),
    fim = lubridate::force_tz(fim, "America/Sao_Paulo")
  ) %>%
  select(-inicio_texto, -fim_texto)  # Limpar colunas temporárias
