...
partidas <- partidas_du %>% 
  bind_rows(partidas_sab,partidas_dom) %>% 
...
partidas_adj <- partidas %>%
  group_by(trip_short_name, fx_hora_agp) %>%
  summarise(total_partidas = sum(partidas),
            total_extensao = round(sum(extensao)/1000,2)) %>%
  rbind(partidas_pf) %>%
  arrange(trip_short_name, fx_hora_agp) %>%
  pivot_wider(names_from = fx_hora_agp, values_from = c(total_partidas, total_extensao), values_fill = list(total_partidas = 0,total_extensao=0)) %>% 
  rename(Serviço = trip_short_name) %>%
  rename_with(~ gsub("total_partidas", "Partidas entre ", .x)) %>%
  rename_with(~ gsub("total_extensao", "Quilometragem entre ", .x)) %>%
  ...
  rename_with(~ gsub("_3_", " 03h e 06h ", .x)) %>% 
  rename_with(~ gsub("_6_", " 06h e 09h ", .x)) %>% 
  rename_with(~ gsub("_9_", " 09h e 12h ", .x)) %>% 
  rename_with(~ gsub("_12_", " 12h e 15h ", .x)) %>% 
  rename_with(~ gsub("_15_", " 15h e 18h ", .x)) %>% 
  rename_with(~ gsub("_18_", " 18h e 21h ", .x)) %>% 
  rename_with(~ gsub("_21_", " 21h e 24h ", .x)) %>%  
  rename_with(~ gsub("_24_", " 24h e 03h (dia seguinte) ", .x)) %>% 
  rename_with(~ gsub("_0_", " 00h e 03h ", .x)) %>%  
...
fwrite(partidas_adj, paste0("C:/R_SMTR/dados/os/os_porhora_", ano_gtfs, "-", mes_gtfs, "-", quinzena_gtfs, "_pf.csv"),sep=';',dec=',')
...
# Salvar o arquivo
saveWorkbook(wb, paste0("C:/R_SMTR/dados/os/os_porhora_", ano_gtfs, "-", mes_gtfs, "-", quinzena_gtfs, "_pf.xlsx"), overwrite = TRUE)
