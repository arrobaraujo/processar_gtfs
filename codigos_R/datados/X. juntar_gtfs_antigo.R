...
gtfs_sppo$routes <- as.data.table(gtfs_sppo$routes) %>%
  ...
  mutate(route_type = if_else(route_short_name == "LECD124", "200", route_type)) %>%
  mutate(route_type = if_else(route_short_name == "LECD125", "200", route_type)) %>%###Para frescões que vier como LECD
  select(-c(numero))
