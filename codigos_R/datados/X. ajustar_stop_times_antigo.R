pacman::p_load(dplyr, gtfstools, lubridate, bizdays, data.table, tidyverse, tibble, tidyr, hms)
...
gtfs$stop_times <- gtfs$stop_times %>% 
  mutate(
    arrival_time = if_else(
      is.na(arrival_time) | arrival_time == "" | arrival_time == "00:00:00",
      as_hms(round(shape_dist_traveled / 3.6)),
      parse_hms(arrival_time)
    ),
    departure_time = arrival_time  # Sempre igual ao arrival_time
  )
...
stp_tms <- gtfs$stop_times %>% 
  ...
  group_by(trip_id) %>% 
  mutate(
    # Criar uma coluna temporária em vez de sobrescrever start_time
    start_time_calc = if_else(
      is.na(start_time), 
      as.character(head(arrival_time, 1)), 
  ...
  ungroup() %>% 
  # Use start_time_calc em vez de start_time nas operações seguintes
  mutate(
    hora = if_else(
      lubridate::hour(lubridate::hms(start_time_calc)) > 23,
      lubridate::hms(start_time_calc) - lubridate::hours(24),
      lubridate::hms(start_time_calc)
    )
  ) %>% 
